...
<script>
  import { queryProtocolTreeList } from '@/api/api'
  ...
  export default {
    ...
    methods: {
      ...
      queryTreeData(keyword) {
        this.commonRequestThen(queryProtocolTreeList({
...
*/
      handleTreeSelect(selectedKeys, event) {
        if (selectedKeys.length > 0 && this.selectedKeys[0] !== selectedKeys[0]) {
          ...
          let protocolId = event.node.dataRef.value
          this.emitInput(protocolId)
        }
      },

      emitInput(protocolId) {
        this.$emit('input', protocolId)
      },
